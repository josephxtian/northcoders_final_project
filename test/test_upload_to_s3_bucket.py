...
class TestWriteToFile:
    def test_return_error_when_no_bucket(self):
        with mock_aws():
            ...
            res = write_to_s3_bucket(
                s3_client, bucket_name, list_of_tables, reformat_data_to_json
            )
    ...
    def test_creates_correct_no_of_files_in_s3(self):
        with mock_aws():
            ...
            assert (
                s3_client.list_objects_v2(Bucket=bucket_name)["KeyCount"] == 5
            )  # note - this is 4 json files and one txt
            assert (
                res == """success - 1 database tables
            have been written to test_bucket!"""
            )
    ...
    def test_contents_of_file_test_data_in_s3(self):
        with mock_aws():
            ...
            for i in range(
                ...
                ["KeyCount"] - 1
            ):  # not the last file as that is the txt
                file_name = s3_client.list_objects_v2(Bucket=bucket_name)\
                ["Contents"][i]["Key"]
